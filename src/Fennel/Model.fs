...
module Timestamp =
    ...
    let asString ts = (date ts).ToUnixTimeMilliseconds().ToString()
...
module Line =
    ...
    let asString line =
        ...
        | Metric m ->  // value is Nan?
            match m with
            | { Name = MetricName name; Labels = []; Value = v; Timestamp = None } -> sprintf "%s %s" name (MetricValue.asString v)
            | { Name = MetricName name; Labels = []; Value = v; Timestamp = Some ts } -> sprintf "%s %s %s" name (MetricValue.asString v) (Timestamp.asString ts)
            | { Name = MetricName name; Labels = labels; Value = v; Timestamp = None } -> sprintf "%s%s %s" name (Label.asString labels) (MetricValue.asString v)
            | { Name = MetricName name; Labels = labels; Value = v; Timestamp = Some ts } -> sprintf "%s%s %s %s" name (Label.asString labels) (MetricValue.asString v) (Timestamp.asString ts)