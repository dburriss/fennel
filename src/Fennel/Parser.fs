...
module PLang =
    ...
    let private pname =
        manyChars2 (asciiLetter <|> underscoreOrColon) (ascii_alpha_numeric <|> underscoreOrColon)
    // Comments parsers
    ...
    let private metric_value =
        (pfloat |>> FloatValue) <|> (skipString "Nan" >>. preturn Nan)

    let private metric_timestamp =
        let withTS =
            pint64
            |>> (fun x -> x |> DateTimeOffset.FromUnixTimeMilliseconds |> Timestamp |> Some)
    ...
    let private a_label_pair =
        ((ws0 >>. pname .>> ws0) .>> (skipChar '=') .>>. (ws0 >>. label_value))
        |>> Label
